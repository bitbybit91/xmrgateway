...
// Validate invoice ID characters (AcceptXMR uses base64url-like IDs)
...
def step_write_site_files(cfg: dict, dry_run: bool, force: bool) -> None:
    ...
    config_php = (
        ...
        .replace("PLACEHOLDER_API_TOKEN",    cfg["api_token"])
    )
...
def step_deploy_acceptxmr(cfg: dict, dry_run: bool) -> None:
    ...
    # 8c. Write .env with secrets
    env_content = (
        f"PRIVATE_VIEWKEY={cfg['viewkey']}\n"
        f"INTERNAL_API_TOKEN={cfg['api_token']}\n"
    )
    write_file(ACCEPTXMR_ENV, env_content, mode=0o600, dry_run=dry_run, force=True)
...
def main() -> None:
    ...
    if onion not in ("pending", "<dry-run — .onion not available>"):
        ...
        step_write_site_files(cfg, dry, force=True)
